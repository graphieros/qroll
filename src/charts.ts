...
export function createCharts() {
    ...
    Array.from(children).forEach(child => {
        ...
        if ((child as HTMLElement).dataset.type === Chart.LINE) {
            ...
            function checkDirtyInputs(input: string | undefined) {
                if (!input) return;
            ...
            const minValue = Math.min(...Object.values(xValues).flatMap(serie => Math.min(...serie as any)));
            ...
            if (showTicks) {
                ...
                for (let i = 0; i < maxDatapoints; i += 1) {
                    ...
                    addTo(labelX, SvgAttribute.Y, calcY(Math.abs(minValue)) + 30);
            ...
            plots.forEach((dataset, index) => {
                ...
                dataset.serie.forEach((plot: any, i: any) => {
                    if (showPlotValue) {
                        ...
                        dataLabel.innerHTML = plot.value;
                    ...
                    if (Array.from((e.target as HTMLElement).classList).includes(CssClass.CHART_TOOLTIP_TRAP) || Array.from((e.target as HTMLElement).classList).includes(CssClass.CHART_DATALABEL_X)) {
                        const tooltipSet = plots.map((plot, i) => {
                            ...
                            return {
                                ...
                                name: plot.name,
                                value: plot.serie[Number((e.target as HTMLElement).dataset.index)]?.value || null,
            ...
            if (hasSubtitle) {
                ...
                addTo(subtitle, SvgAttribute.Y, 60);
